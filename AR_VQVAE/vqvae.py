...
class TemporalVertexCodec(nn.Module):
    def __init__(self, n_vertices: int = 338, latent_dim: int = 128, categories: int = 128, residual_depth: int = 4):
        ...
        self.n_vertices = n_vertices * 3
        self.encoder = TemporalVertexEncoder(n_vertices=self.n_vertices, latent_dim=latent_dim) 
        self.decoder = TemporalVertexDecoder(n_vertices=self.n_vertices, latent_dim=latent_dim) 
    ...
    def forward(self, verts, mask=None):
        ...
        verts = verts.view(B, T, -1) # [B, T, NC] 
        x = self.encoder(verts) # x: [B T 64] 
